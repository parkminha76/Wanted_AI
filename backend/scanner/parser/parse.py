...
# 스캔본 PDF를 그림으로 구울 때의 배율. 1.0 = 72dpi, 1.5 = 108dpi.
#
# 1.5인 이유: YOLO는 입력을 640px로 줄여서 본다. 72dpi로 구우면 A4가 595px라 이미
# 그보다 작아서, 신분증처럼 페이지 일부만 차지하는 대상은 글자 영역이 뭉개진다.
# 1.5배만 돼도 A4 폭이 892px로 640을 넉넉히 넘어 이 문제는 해결된다.
#
# 원래 2.0(A4 1190px)이었는데, 실측(2026-09-20)으로 이 그림을 그대로 받는
# text_ocr(EasyOCR)이 해상도에 아주 민감하다는 게 드러났다 — 같은 스캔본
# 5페이지 PDF가 2.0배에서 114초, 페이지당으로 보면 2.0배가 1.5배보다 30%
# 넘게 느렸다(직접 배율 비교 실측). id_detector(YOLO)가 필요로 하는
