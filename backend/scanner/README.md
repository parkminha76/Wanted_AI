...
| 2026-09-17 | 위 수정의 부작용: 앵커가 없으면 findings를 통째로 버리다 보니, 신분증이 아닌 이력서·지원서의 증명사진(face) 자체도 마스킹에서 완전히 빠져버림 — 얼굴도 가려야 할 개인정보인데 안 가려짐 | 앵커 판정("이게 신분증인가")과 "얼굴을 가릴지"를 같은 all-or-nothing 판정 하나로 묶어서 처리했다. 문제였던 오탐(주소·이름이 잘못 잡히는 것)은 전부 글자 영역 클래스였지 얼굴이 아니었다 | `id_detector.py`에 `_KEEP_WITHOUT_ANCHOR = {"face"}` 추가 — 앵커가 없어도 얼굴만은 남기고, 앵커가 있어야 믿을 수 있는 나머지 클래스(address/name/signature/id_meta 등)만 버린다 |
...
| 2026-09-17 | 위 수정의 부작용: 지원서 "직장명" 표에서 그 왼쪽에 세로 선으로 나뉜 별도 병합 셀(여러 행에 걸친 행 그룹 라벨 "아르바이트\n경력사항")의 글자가 회사명 값으로 잘못 잡혀, 라벨 자체가 마스킹으로 가려짐(사용자가 스크린샷으로 발견) | 첫 열의 왼쪽 끝을 무조건 0(이미지 왼쪽 끝)으로 열어 뒀다 — 그 사이 어딘가에 있다는 이유만으로 완전히 다른(표 바깥) 셀의 글자까지 그 열의 값으로 삼켰다. 이 표는 그 행의 실제 값("A식품" 등)을 OCR이 아예 못 읽어서, 라벨 글자만 유일한 후보로 남아 있었다 | `_column_boundaries`가 첫 열·마지막 열도 그 열 자신의 폭(반대쪽 이웃까지 거리)의 절반만큼만 바깥으로 열어 두도록 수정 — 표의 실제 테두리를 몰라도 "이 정도까지가 합리적인 열 범위"로 제한한다 |
