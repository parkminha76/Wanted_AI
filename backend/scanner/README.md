...
| (부분 해결, 미완) | 위 표의 "A식품"/"B식품"/"C식품" 값 자체가 마스킹되지 않음 — OCR이 이 세 글자를 `--psm 3/4/6/7/8/11/12`·배율 1~4배·`kor+eng`/`eng` 조합을 전부 시도해도 못 읽는다(영문 대문자 한 글자와 한글이 공백 없이 붙은 토큰이 원인으로 보임). OCR이 값 자체를 raw_text에 못 남기면 이 뒤 어떤 판정 로직도 손쓸 수 없다 | 값을 못 읽어도 표 구조상 그 칸에 무언가 있어야 한다는 건 알 수 있어서, `_find_table_column_cells`에 "값 없이 자리만 방어적으로 가리는" 기능을 추가했다(id_detector.py가 얼굴 영역을 값 없이 좌표만으로 가리는 것과 같은 방식). 다만 이 문서에서는 표 첫 행("A식품")부터 OCR이 실패해서, "실패한 행 하나를 건너뛰고 다음 행부터 계속 훑기"도 시도했으나 **표 끝을 못 찾아 자기소개서·서명란까지 회사명으로 잘못 잡는 훨씬 심한 회귀**가 나서 폐기했다. 표 중간 행 하나가 안 읽혀도 첫 실패에서 멈추는 지금 동작이 더 안전하다고 판단 | `_find_table_column_cells`/`_collect_column_rows`에 방어적 자리 마스킹 기능은 남겨 뒀다(다른 문서에서 표 첫 행은 멀쩡하고 중간 행만 이런 경우 도움이 됨) — 이 문서의 첫 행 실패 자체를 넘어서려면 단어 위치 추정이 아니라 실제로 그려진 표 테두리 선(`_remove_table_lines`가 이미 계산하는 가로/세로 선 위치)으로 표의 진짜 끝을 판단해야 하는데, 그건 별도로 계획해서 진행할 큰 작업이라 미착수 |
| 2026-09-18 | 위 항목들을 psm 전환·재시도·간격 보정 등으로 하나씩 막아 왔는데도 캔바·미리캔버스류 이력서 이미지에서 계속 새로운 자리가 깨짐 — 히어로 타이틀 이름이 완전히 다른 글자로 읽히고, 페이지를 한 번에 읽을 때만 특정 줄의 특정 글자가 사라지고, 위치 핀 아이콘 옆 "서울특별시"의 "서"가 통째로 없어지고, 장식 아이콘이 "ITQAAS AS — \|" 같은 글자로 오인식돼 인젝션 분류기까지 오탐시킴 | 이 모든 증상의 근본 원인은 Tesseract 자체였다 — 스캔한 문서·인쇄물처럼 "밝은 배경 + 어두운 글씨"인 단순한 이미지를 전제로 만들어진 엔진이라, 화려한 배경 무늬·장식 폰트·아이콘이 섞인 그래픽 디자인에는 태생적으로 약하다(배포 환경에서는 Tesseract 엔진 버전(5.5.0 vs 5.5.3)만 달라도 같은 이미지를 다르게 읽는 사고까지 있었다 — 위 표 참고). 같은 이미지 2장을 EasyOCR(딥러닝 기반)로 돌려보니 위 증상이 한 번에 다 해결됨을 실측으로 확인 | `text_ocr.py`를 Tesseract에서 EasyOCR로 전면 교체. `--psm` 재시도·`_deskew`(회전 보정)·`_recover_gap_lines`(빈 구간 재시도)·`_remove_table_lines`(표 테두리 제거)·`_merge_adjacent_syllables`(음절 재조합) 등 Tesseract 특유의 실패를 우회하던 코드를 전부 걷어냈다 — EasyOCR의 검출기가 회전·배경 잡음에 원래 강해서 필요 없어졌다. `_find_table_column_cells` 등 엔진과 무관한 기하학적 로직(표 열 인식, 라벨-값 이어붙이기)은 그대로 재사용. Dockerfile의 Tesseract 소스 빌드 블록도 제거 |
| 2026-09-18 | 모바일로 노트북 화면을 세로로 세워 찍은 사진(90도 돌아간 채로 업로드)에서 마스킹이 거의 통째로 빠짐 — 이름 3곳 중 0곳, 주소·전화·생년월일 전부 노출 | 사진이 물리적으로 회전된 채로 들어오면 EasyOCR이 거의 못 읽는다. 낱글자 하나짜리 검출은 우연히 확신도가 높게 나올 수 있어("0" 0.99 등) 확신도만으로는 방향을 못 가늠한다 | `_ocr_lines`에 회전 복구 추가 — 기본 방향에서 2글자 이상·확신도 0.6 초과 검출의 글자 수 합이 낮을 때만(흔한 정상 문서는 매번 추가 비용 없이 그대로 감) 90/180/270도로 다시 읽어 제일 잘 읽히는 각도를 고른다. 이후 모든 구조 판정(줄 묶기·표 인식)은 이 "바로 세운" 좌표계에서 돌고, 마스킹 직전에만 원본 파일 좌표로 되돌린다(`_map_bbox_from_rotated`) |
| 2026-09-18 | 회전 복구된 사진에서 이름 bbox(세로로 선 글자라 가로 53px·세로 148px)에 여백을 주면서, "최태오" 바로 뒤에 이어진 "의 발자취"까지 뭉텅 가려짐(세 곳 전부) | 텍스트 필드 여백 로직이 "글자는 항상 가로로 눕는다"를 가정해 세로 축에 무조건 30%(주소는 55%) 여백을 줬다. 세로로 선 글자는 그 축이 읽는 방향이라, 148*0.30≈44px가 옆 단어까지 침범했다 | `_padded_image_box`를 상자 자신의 가로세로 비율로 "줄 두께 방향"(넉넉히)과 "읽는 방향"(최소한만) 축을 판정하도록 수정 — 가로로 눕는 문서에서는 원래 동작과 수학적으로 동일 |
| 2026-09-18 | 표에서 확정된 값이 자유 서술문에도 그대로 나오면 같이 잡는 교차 참조 로직에서, 인접한 두 값(예: "A식품"·"B식품")의 보간된 bbox가 서로 겹칠 때 뒤에 처리된 값이 먼저 넣은 값을 지워버림 | `_merge_table_cells`가 table_cells를 하나씩 순차 append하며 겹침 검사를 했는데, 그 검사가 findings뿐 아니라 방금 넣은 다른 table_cells와도 겹쳐 서로를 밀어냈다 | table_cells끼리는 서로 지우지 않도록 수정 — 겹침 제거는 기존 findings에만 적용 |
| 2026-09-18 | 표의 "직장명"에서 "C식품"이 자유 서술문에서는 "("로 오독되어("(식품 공장에서...") 교차 참조가 못 찾음 | EasyOCR이 "C"를 소문자 괄호와 혼동했다(확신도 0.459로 실제로 "(식품"이라고 읽음) | `_find_cross_referenced_values`에 폴백 추가 — 표 셀 자기 위치 말고는 정확히 일치하는 자리가 하나도 없을 때만, "영문 한 글자 + 한글 단어" 값의 그 글자를 흔한 OCR 오독 기호(괄호·숫자)로 바꾼 형태도 찾는다 |
| 2026-09-18 | 저해상도 이력서 사진(865f267df9c220bf.jpg, 412×616)의 "경력사항" 표에서 회사명 "Fauget"이 전혀 안 가려짐 | 이 표는 열 헤더 자체가 저해상도로 깨져("회사명"/"경력"/"소속" 대부분 유실, "기간"만 읽힘) `_find_table_column_cells`가 표로 인식을 못 하고, 목록형 폴백(`_find_career_list_entries`)으로 넘어갔는데, 그 폴백은 "연도-연도"가 **두 칸으로 따로 검출된** 행만 인식했다. 이 문서는 같은 표 안에서도 어떤 줄은 "2022"/"2023"으로 갈리고, 어떤 줄("2024 * 2025", 하이픈이 별표로 오독)은 한 칸으로 통째로 검출돼 아예 인식되지 않았다 | `_find_career_list_entries`에 "연도-연도가 한 칸으로 통째로 검출된 경우"도 인식하는 분기 추가(`_YEAR_RANGE_MERGED`) — 단, 뒤에 최소 두 칸이 더 있어야 한다(한 칸만 남으면 그게 회사명인지 다른 열 내용인지 구분 불가) |
| 2026-09-18 | 위 수정 직후 부작용: "회사명"란 값("FauBct")이 176px 떨어진 "소속"란 값("디자인터")까지 한 값으로 뭉쳐 잡혀, 마스킹 박스가 "경력"란(업무 설명)까지 통째로 덮음 — 사용자가 스크린샷으로 발견 | 목록형 폴백은 "연도 뒤에 오는 글자를 전부" 회사명으로 본다는 전제로 만들어졌는데(진짜 목록형 문서는 회사명 다음에 다른 열이 없다), 이 문서는 실제로는 표라 열 사이에 간격이 있다 | 칸 사이 가로 간격이 줄 높이의 5배를 넘으면 그 뒤는 다른 열로 보고 자르도록 수정(`_CAREER_LIST_VALUE_GAP_RATIO`) — 진짜 여러 단어짜리 회사명은 EasyOCR이 애초에 한 덩어리로 검출해 이 컷오프에 안 걸린다 |
| 2026-09-18 | 같은 문서 "자격증" 섹션에서 자격증 이름이 OCR로 "공인 임어시럽"처럼 깨져 읽혔는데, NER이 그 문맥에서 "공인"만 떼어 회사명으로 오판(확신도 0.401)해서 자격증란이 잘못 가려짐 | "공인"은 "공인중개사"·"공인회계사"처럼 항상 뒤에 명사가 붙어야 뜻이 서는 수식어라 단독 개체(회사명)가 되는 일이 사실상 없는데, 주변이 온통 OCR 잡음인 문맥에서 모델이 착각했다. 신뢰도로 거르면 안 된다 — 같은 실측에서 진짜 회사명 "네이버"도 0.364로 이보다 낮게 나왔다 | `ner.py`에 `_ORG_STANDALONE_MODIFIERS` 추가 — 값이 이 작은 목록과 **정확히 일치**할 때만 org 후보에서 뺀다 |
| 2026-09-18 | 이미지 마스킹 박스가 순검정 각진 사각형이라 여러 개 흩어져 있으면 지저분해 보인다는 사용자 피드백 | 디자인 개선 요청(버그 아님) | `mask.py`의 `_IMAGE_FILL`을 순검정에서 중간 회색(140,140,140)으로, `draw.rectangle`을 `draw.rounded_rectangle`로 교체. 모서리 반지름은 그 상자에 실제로 적용된 여백(패딩)보다 항상 작게 계산해서(`_padded_image_box`가 `(box, radius)`를 함께 반환) — 안 그러면 얼굴처럼 여백이 2px뿐인 상자에서 모서리를 깎다가 얼굴 가장자리가 그대로 드러난다 |
...
`TableColumnSchoolExclusionIntegrationTest` / `RotationRecoveryTest` /
`MapBboxFromRotatedTest` / `FindCareerListEntriesTest` /
`FindCrossReferencedValuesTest`, `test_masking_policy.py`의
`test_image_finding_selection_with_zero_offsets_is_accepted`, `test_ner_school_exclusion.py`,
`test_ner_org_standalone_modifiers.py`, `test_mask_image.py`가 위에서 고친 사례들의 회귀를 막는다.
...
## 9. 알려진 한계 (이미지 OCR) — 코드로 못 고치는 것

위 표의 항목들은 전부 "실제로 고칠 수 있었던" 사례다. 아래는 실측으로 원인까지
확인했지만, **이미지 자체에 남은 정보가 부족해서** 지금 구조(EasyOCR + 규칙
기반 후처리)로는 못 고치는 것들이다. 본선에서 시간이 더 있다면 다음 후보들이
OCR 모델 자체를 손대야 하는 순서다.

| 한계 | 실측 근거 | 왜 코드로 안 되는가 |
|---|---|---|
| **화면을 카메라로 재촬영한 사진(모아레)** | 노트북 화면을 휴대폰으로 찍은 사진에서 이름은 회전 복구로 잡혔지만, 그 옆 주소·전화·이메일 구역은 원본·2~4배 업스케일+샤프닝·가우시안 블러+업스케일·미디언 필터+이진화까지 8가지 조합을 다 시도해도 글자를 못 읽음 | LCD 픽셀 격자와 카메라 센서가 간섭해서 생기는 모아레 무늬가 그 구역의 미세한 획 정보 자체를 파괴한다. 업스케일링은 확신도만 올릴 뿐("hellooreallygrcatsite" 0.358→0.739) 실제 글자는 계속 다르게 틀린다 — 없는 정보는 후처리로 복원이 안 된다 |
| **저해상도 원본(예: 412×616)** | 같은 문서에서 이메일 "hello@reallygreatsite.com"이 "hellooreallyErcatsite"로 읽혀 "@"과 ".com"이 통째로 사라짐. 생년월일 "1996.05.24"도 배율(1~4배)을 바꿔가며 시도했지만 매번 다른 숫자로 틀림(34/26/26/28) | "@"처럼 작고 곡선이 많은 글자가 원본 해상도에 애초에 몇 픽셀 안 남아 있다. 저해상도와 모아레는 서로 다른 원인이지만 결과(정보 소실)는 같다 |
| **텍스트 라벨 없이 아이콘만 쓰는 필드** | 위 저해상도 문서에서 생년월일이 달력 아이콘 옆에만 있고 "생년월일" 글자 라벨이 없어서, `find_birth_dates`의 두 단서(앞: "생년월일", 뒤: "\|B형" 같은 혈액형 패턴) 중 어느 쪽도 못 찾아 탐지 자체가 안 됨(최태오.png는 "\|B형" 뒷단서가 있어서 잡혔던 것과 대비) | 날짜 형식 문자열은 경력 기간("2020 - 2021")·발급일자 등과 구분이 안 돼서, 단서 없이 아무 날짜나 생년월일로 잡으면 오탐이 폭증한다(과거에 실측으로 확인된 트레이드오프). 디자인이 단서를 아예 안 주면 텍스트만으로는 원천적으로 구분 불가 |
| **표 헤더 자체가 OCR로 안 읽히는 경우의 열 경계 추정** | 위 문서의 "경력사항" 표는 헤더가 "기간" 한 칸만 읽혀 `_find_table_column_cells`(진짜 열 인식)를 못 쓰고 목록형 폴백으로 넘어간다. 칸 사이 간격 기준(줄 높이의 5배)으로 회사명만 잘라내는 보정을 넣었지만, 그 행의 회사명 자체가 OCR로 아예 안 읽히면(실측: "프리랜서" 행) 그 행은 통째로 놓친다 | 열 경계를 결정할 근거(헤더 텍스트)가 없으니 기하학적 추정일 뿐이고, 애초에 글자가 안 읽힌 칸은 추정할 대상 자체가 없다 |

**공통 결론**: 위 넷 다 "OCR이 애초에 그 글자를 문자열로 못 만들어 낸다"는 같은
뿌리다. 규칙·기하학 기반 후처리(`text_ocr.py`)는 OCR이 **읽어낸** 글자를 더
정확하게 해석하는 역할까지만 할 수 있고, 읽어내지 못한 글자를 복원하지는
못한다. 다음 단계는 이런 열화 패턴(모아레·저해상도)을 흉내 낸 합성 데이터로
OCR 인식기 자체를 파인튜닝하는 것 — 오늘 실패한 실제 사례들이 그 학습 데이터
후보다.
