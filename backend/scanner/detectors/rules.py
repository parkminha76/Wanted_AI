...
# ---------- 카드번호 ----------
# 12~19자리. 브랜드마다 자릿수가 달라서(Visa 16 / Amex 15 / Diners 14) 4-4-4-4로
# 못 박으면 Amex·Diners가 계좌번호로 오분류된다. 넓게 잡고 Luhn으로 걸러낸다 —
# 무작위 숫자열이 Luhn을 통과할 확률은 1/10이라 필터가 충분히 강하다.
CARD_NUMBER_PATTERN = re.compile(r"(?<!\d)(?:\d[-\s]?){11,18}\d(?!\d)")
...
BANK_ACCOUNT_NUMBER_PATTERN = re.compile(
    r"(?<!\d)(?!01[016789][-\s]?\d)(?:\d{2,7}(?:[-\s]\d{2,7}){1,4}|\d{10,16})(?!\d)"
)
