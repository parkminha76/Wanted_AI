...
# 이 스크립트는 **매번 전체 데이터를 다시 만든다.** 그런데 파일명에 날짜가 들어가서,
# 날짜가 바뀐 뒤 다시 돌리면 어제 파일이 그대로 남고 새 파일이 하나 더 생긴다.
#
# 그 상태로 학습하면 **같은 데이터를 두 번 배운다** — false_positive_filter.py가
# `sample_data/false_positive/false_positive_*.json`을 glob으로 전부 읽기 때문이다
# (실측 2026-09-13: 344건짜리 파일 두 개가 남아 688건으로 학습됐다). 교차검증에서
# 같은 문장이 학습과 검증 양쪽에 들어가 성능이 실제보다 높게 나온다.
#
# 그래서 저장할 때 **이 스크립트가 전에 만든 파일은 지운다.** 매번 전체를 다시 만드니
# 잃는 것이 없고, 지운 파일 이름은 화면에 찍어서 조용히 사라지지 않게 한다.
# 다른 사람이 넣은 파일(AUTHOR가 다른 것)은 건드리지 않는다.
...
def _remove_stale(keep_path: str) -> list[str]:
    """이 스크립트가 전에 만든 파일을 지운다. 지운 이름 목록을 돌려준다.

    `false_positive_{AUTHOR}_{MMDD}_{BATCH}.json` 꼴이면서 이번에 쓸 파일이 아닌 것만
    지운다. 다른 사람이 넣은 파일은 이름이 안 맞아서 걸리지 않는다.
    """
    import re

    pattern = re.compile(rf"^false_positive_{re.escape(AUTHOR)}_\d{{4}}_\d{{3}}\.json$")
    removed = []
    if not os.path.isdir(OUT_DIR):
        return removed
    for name in sorted(os.listdir(OUT_DIR)):
        candidate = os.path.join(OUT_DIR, name)
        if candidate == keep_path or not pattern.match(name):
            continue
        try:
            os.remove(candidate)
        except OSError:
...
# ---------------------------------------------------------------------------
#
# 라벨 기준 — **타입이 아니라 민감도다** (2026-09-12 A 문의로 명문화)
# ---------------------------------------------------------------------------
#
#   label=1  이 값을 **가려야 한다**  (진짜 개인정보)
#   label=0  이 값을 **가리지 않아도 된다**
#
# "전화번호 형식이 맞는가"가 아니다. 형식은 rules.py가 이미 판정했고, 분류기는 그
# **뒤에** 불린다(models.filter_false_positive). 같은 질문을 두 번 하면 분류기가 할 일이
# 없다. 계약서도 민감도로 적혀 있다:
#
#     models.filter_false_positive(text, context, risk_type)
#         """이 값이 **진짜 개인정보인가**? 반환: (진짜면 True, 진짜일 확률 0~1)"""
#
#     scan.py: is_real=False -> filtered_out 으로 간다 = **마스킹하지 않는다**
#
# 그래서 "고객센터 1588-1234"는 전화번호가 맞지만 label=0이다. 회사 안내문의 대표번호를
# 전부 [전화번호]로 덮으면 사본을 쓸 수 없다.
#
# **타입이 헷갈리는 것과 민감도가 헷갈리는 것은 타입마다 다르다.** account·card는
# 타입부터 애매하고(주문번호·상품권번호), phone은 타입은 확실한데 주인이 애매하다.
# 분류기는 어느 쪽이든 "가려야 하나"라는 한 가지 질문에만 답한다.
#
# 규칙 네 가지
#   1. 값이 들어갈 자리에 `{value}`를 **정확히 한 번** 쓴다.
#   2. label=1은 "가려야 하는 문장", label=0은 "안 가려도 되는 문장"(위 기준).
#   3. **문장을 다양하게 쓴다.** 모델은 숫자가 아니라 옆에 있는 단어를 본다.
#      같은 문장에 값만 바꾸면 모델에게는 같은 문장 하나다.
#   4. **단, 판단 근거가 되는 단어는 문장 3개 이상에 반복시킨다.** (2026-09-12 추가)
#      3번만 지키면 근거 단어가 전부 문장 하나에만 나와서, 평가가 안 본 문장을
#      요구할 때 모델이 기댈 것이 없어진다. 아래 실측 참고.
#   5. **라벨과 상관없는 단어는 라벨 간에 고르게 섞는다.** (2026-09-12 추가)
#      rules.py가 잡으려면 반드시 들어가야 하는 앵커 단어(사번/사원번호/직원번호)가
#      한쪽 라벨에 몰리면, 분류기가 문맥이 아니라 그 단어만 보고 답을 맞힌다.
#      실제 문서에서는 앵커 단어와 진위가 아무 상관이 없으므로 배우면 안 되는 규칙이다.
#
...
TEMPLATES: dict[str, dict[int, list]] = {
    ...
    # ---------------- 사번 ----------------
    "emp_no": {
        # 근거 단어를 묶음별로 반복시킨다(규칙 4). 여기서 근거는 "그 번호에 **사람
        # 한 명이 붙어 있다**"를 드러내는 말이다 — 직원, 계정, 발급, 신청처럼 개인의
        # 행위·소유를 가리키는 단어.
        #
        # **앵커 키워드(사번/사원번호/직원번호)는 라벨 간에 고르게 섞는다** (규칙 5).
        # 2026-09-12 실측: label 1이 "사번" 16문장인데 label 0은 "사원번호"·"직원번호"에
        # 몰려 있었다. 그러면 분류기가 문맥이 아니라 **앵커 단어만 보고** 답을 맞힌다 —
        # 실제 문서에서 앵커 단어와 진위는 아무 상관이 없으므로 배우면 안 되는 규칙이다.
        1: [
            # --- "직원/사원" 묶음: 값 뒤에 사람이 온다 ---
            "사번 {value} 직원의 근태 기록입니다",
            "직원번호 {value} 소지자의 연차 잔여일수를 확인합니다",
            "사원번호 {value} 직원의 부서 이동을 승인합니다",
            "직원번호 {value} 본인의 재직 기간을 조회했습니다",
            # --- "계정/로그인" 묶음 ---
            "입사 시 부여된 사번 {value}로 로그인하세요",
            "퇴사 처리된 사원번호 {value} 계정을 비활성화합니다",
            "사번 {value} 계정의 비밀번호가 초기화되었습니다",
            "직원번호 {value} 계정으로 사내망에 접속했습니다",
            # --- "발급/부여" 묶음 ---
            "급여명세서는 사원번호 {value} 기준으로 발급됩니다",
            "직원번호 {value} 경력 증명서를 발급했습니다",
            "신규 입사자에게 사번 {value}를 부여했습니다",
            "재직증명서를 사원번호 {value}로 발급 신청했습니다",
            # --- "신청/등록" 묶음 ---
            "인사 시스템에 사번 {value}가 등록되어 있습니다",
            "사원번호 {value}로 사내 시스템 권한을 신청했습니다",
            "출입카드 재발급 대상 직원번호는 {value}입니다",
            # --- 개인 일정·기록을 가리키는 문장 ---
            "교육 이수 현황 — 사번 {value} 미이수",
            "평가 대상자 사원번호 {value}의 면담 일정입니다",
            "직원번호 {value} 님의 건강검진 예약이 완료되었습니다",
        ],
        0: [
            # ⚠️ 이 칸은 2026-09-12에 두 번 고쳤다.
            #
            # rules.py의 emp_no 탐지기는 **키워드 앵커 방식**이다 —
            # (사번|사원번호|직원번호|임직원번호) 바로 뒤에 오는 값만 잡는다.
            # 그래서 "문서번호 97-83140" 같은 문장은 애초에 후보가 되지 않는다
            # (맨 처음 15문장이 전부 그런 것들이라 30건이 통째로 쓸모없었다).
            #
            # 이 방식에서 오탐이 나는 자리는 하나뿐이다: **사번 키워드 뒤에 오는 값이
            # 실제 직원의 번호가 아닌 경우** — 형식 설명, 예시, 더미값, 번호 대역.
            #
            # 두 번째 수정(A 지적): "사번 {value} 이후 입사자부터 새 체계를 적용합니다"를
            # 뺐다. 그 값은 **실제 직원의 사번을 기준점으로 쓴 것**일 수 있어서 개인정보
            # 여부가 문장만으로 안 갈린다. 이 파일의 경계선 방침("애매하면 쓰지 않는다")에
            # 걸리는 문장이었다.
            #
            # 근거 단어("형식"·"예시"·"더미"·"대역")를 묶음별로 3개 이상 반복시킨다(규칙 4).
            # --- "형식/체계" 묶음 ---
            ("사번 {value} 형식으로 자동 부여됩니다", "사번 형식 설명"),
            ("직원번호 {value} 형태는 2020년부터 쓰지 않습니다", "폐기된 형식"),
            ("사원번호 {value} 형식은 내년부터 8자리로 바뀝니다", "변경될 형식"),
            ("사번 {value} 형식 설명을 인사규정에 추가했습니다", "형식 설명"),
            # --- "예시/샘플" 묶음 ---
            ("직원번호 {value} 예시를 참고해 작성해 주세요", "작성 예시"),
            ("사번 {value} 는 샘플이며 실제 직원이 아닙니다", "샘플값"),
            ("사번 {value} 마스킹 처리 예시 화면입니다", "마스킹 예시"),
            ("사원번호 {value} 예시로 양식을 미리 채워 두었습니다", "작성 예시"),
            # --- "더미/테스트" 묶음 ---
            ("사번 {value} 컬럼은 테스트 값으로 채워져 있습니다", "테스트 데이터"),
            ("직원번호 {value} 더미값으로 이관 테스트를 진행했습니다", "더미값"),
            ("사번 {value} 테스트 계정은 운영에 반영하지 않습니다", "테스트 계정"),
            ("사원번호 {value} 더미 데이터를 삭제해 주세요", "더미값"),
            # --- "대역/규칙" 묶음 ---
            ("사원번호 {value} 대역은 아직 사용하지 않습니다", "미사용 번호대역"),
            ("사번 {value} 번대는 재사용하지 않습니다", "번호대역 정책"),
            ("직원번호 {value} 규칙을 이번에 개정했습니다", "부여 규칙"),
            ("사번 {value} 대역은 계열사에 할당된 구간입니다", "번호대역 할당"),
            # --- "템플릿/양식" 묶음 ---
            ("사원번호 {value} 템플릿을 내려받아 작성하세요", "양식 템플릿"),
            ("직원번호 {value} 양식 파일을 공유드립니다", "양식 템플릿"),
        ],
    },
    # ---------------- 전화번호 ----------------
    #
    ...
    "phone": {
        # 근거 단어를 묶음별로 3개 이상 반복시킨다(규칙 4). 2026-09-12 이전에는
        # 문장 하나에 근거 단어 하나씩이라 82%가 한 번만 나왔고, 안 본 문장에서
        # 모델이 기댈 것이 없었다.
        1: [
            # --- "직통" 묶음: 개인에게 바로 연결된다 ---
            "담당자 직통번호는 {value}입니다",
            "김 과장 직통 {value}로 바로 연결됩니다",
            "직통번호 {value}는 담당자 본인만 받습니다",
            # --- "본인/제 명의" 묶음 ---
            "비상연락망에 본인 연락처 {value}를 등록했습니다",
            ("본인 명의 휴대전화 {value}로 본인확인을 진행합니다", "", "phone_mobile"),
            "제 명의로 되어 있는 번호는 {value}입니다",
            "본인 연락처가 {value}로 변경되었습니다",
            # --- "고객님/수령인/보호자" 묶음: 특정 개인을 가리킨다 ---
            "예약 확인을 위해 고객님 번호 {value}로 연락드리겠습니다",
            ("택배 수령인 연락처: {value}", "", "phone_mobile"),
            "보호자 연락처 {value}를 기재해 주세요",
            "고객님께서 남기신 번호 {value}로 회신드립니다",
            # --- "휴대폰/개인" 묶음 ---
            ("휴대폰 번호 {value}로 인증번호를 발송했습니다", "", "phone_mobile"),
            ("긴급 시 담당자 개인 번호 {value}로 전화 부탁드립니다", "", "phone_mobile"),
            ("개인 휴대전화 {value}로 알림톡이 전송되었습니다", "", "phone_mobile"),
            # --- "자택" 묶음 ---
            "재택근무 중이라 자택 전화 {value}로 연락 부탁드립니다",
            "자택 번호 {value}는 저녁에만 받습니다",
            # --- 키워드 없이 문맥만으로 개인 소유가 드러나는 문장 ---
            # 위 묶음 단어로만 label 1을 채우면 모델이 그 단어들을 외운다. 그 단어가
            # 없는 개인 번호를 통째로 놓치게 되므로 섞어 둔다.
        ...
        0: [
            ...
            ("제품 문의는 고객센터 {value}를 이용해 주세요", "회사 고객센터", "phone_landline"),
            ("고객센터 운영시간 외에는 {value}로 연결됩니다", "회사 고객센터", "phone_landline"),
            # --- "대표번호" 묶음 ---
            ...
            ("상담원 연결은 콜센터 {value}를 눌러 주세요", "콜센터", "phone_landline"),
            # --- 부서 공용 번호 묶음 ---
            # "기술지원 직통"은 뺐다. label 1의 "담당자 직통"과 **같은 단어로 반대
            # 라벨**이 되어, 문장이 적은 지금은 "직통"을 잡음으로 만든다(A 지적,
            # 2026-09-12). 부서 소유라는 사실은 "기술지원"·"팀"만으로 충분히 드러난다.
            ("기술지원 문의는 {value}로 접수해 주세요", "부서 공용번호", "phone_landline"),
            ("총무팀 사무실 번호 {value}로 문의 바랍니다", "부서 사무실번호", "phone_landline"),
            ("구매팀 공용 번호 {value}로 발주서를 보내주세요", "부서 공용번호", "phone_landline"),
            # --- 공개·접수 묶음 ---
            ...
            ("매뉴얼 캡처에 쓰인 {value}는 가상의 예시입니다", "매뉴얼 예시"),
        ],
    },
    # ---------------- 카드번호 ----------------
    "card": {
...
_ACCOUNT_SHAPES = ((3, 2, 4, 5), (3, 3, 6), (3, 3, 7), (4, 3, 6), (3, 6, 5), (4, 4, 5), (3, 4, 6))
...
def main() -> int:
    ...
    for name in removed:
        print(f"  묵은 파일 삭제: {name}  (이 스크립트가 전에 만든 것 — 두면 중복 학습된다)")
    if not ready:
